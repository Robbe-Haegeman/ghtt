...
@click.option("--at", "-a", help="Time at which to show repository")
@click.option(
    "--rm-repo",
    "-r",
...
def branches_to_folders(source, at=None, rm_repo=False):
    ...
    if os.path.exists(f"{source}.expanded"):
        click.secho(
            f"ERROR: the path '{source}.expanded' already exists. Please remove that directory first.",
            fg="red",
        )
        return False
    ...
    for branch in branches:
        ...
        subprocess.check_call(
            ["git", "clone", "--single-branch", "--branch", branch, source, destination]
        )
        if at:
