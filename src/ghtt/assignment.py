...
def _check_repo_groups(
    yes: bool, repos: dict[str, StudentRepo]
) -> dict[str, StudentRepo]:
    ...
    for repname, repo in repos.items():
        if (expected_group_size and len(repo.students) != expected_group_size) or (
            expected_mentor_count and len(repo.mentors) != expected_mentor_count
        ):
            if expected_mentor_count or repo.mentors:
                click.secho(
                    "Group {} has {} students and {} mentors (expected {}/{}):".format(
                        repo.group,
                        len(repo.students),
                        len(repo.mentors),
                        expected_group_size,
                        expected_mentor_count,
                    )
                )
            else:
                click.secho(
                    "Group {} has {} students (expected {}):".format(
                        repo.group, len(repo.students), expected_group_size
                    )
                )
...
@click.pass_context
@click.option(
    "--branch", help="Name of the branch to create in students repos", required="True"
)
@click.option("--title", help="Title of the pull request.", required="True")
@click.option("--body", help="Body of the pull request (the message).", required="True")
@click.option(
    "--source",
    "-s",
...
@click.option(
    "--students", help="Comma-separated list of usernames. Defaults to all students."
)
@click.option(
    "--groups", help="Comma-separated list of group names. Defaults to all groups."
)
@click.option(
    "--yes", help="Process all students/groups, without confirmation.", is_flag=True
)
def create_pr(
    ctx,
    branch,
    ...
    for repo in repos.values():
        try:
            ...
            click.secho(
                "Warning: repository {} not found, skipping".format(repo.url),
                fg="yellow",
            )
            continue
        ...
        if not branch_already_pushed:
            ...
            pr = g_repo.create_pull(
                title=title, body=body, base=default_branch, head=branch
            )
...
def generate_file_from_template(path, clone_url, repo: ghtt.config.StudentRepo):
    ...
    try:
        ...
        click.secho(
            f"Problem generating template for path={path} clone_url={clone_url}",
            fg="red",
        )
        raise
...
def create_repos(ctx, source, yes, students=None, groups=None):
    ...
    if not Path(f"{source}/.git").exists():
        click.secho(
            "The template source ({}) does not contain a .git repository. Make sure to initialize the template repository.".format(
                source
            ),
            fg="red",
        )
    ...
    for repo in repos.values():
        try:
            g_repo = g_org.get_repo(repo.name)
            click.secho(
                "Warning: repository {}/{} already exists; skipping..".format(
                    g_org.html_url, repo.name
                ),
                fg="yellow",
            )
            continue
        ...
        try:
            ...
            click.secho(
                f"The branch `{default_branch}` does not exist in the source repository. Please specify the correct source branch in `ghtt.yaml` using the `default-branch` keyword."
            )
        ...
        for path in Path(source).rglob("*.jinja"):
            generate_file_from_template(path, clone_url=g_repo.clone_url, repo=repo)
        ...
        subprocess.check_call(
            ["git", "push", g_repo.ssh_url, f"{repo.name}:{default_branch}"], cwd=source
        )
        subprocess.check_call(
            ["git", "checkout", default_branch], cwd=source
        )  # go back to source branch

        click.secho(
            f"Protecting the {default_branch} branch so students can't rewrite history",
            fg="green",
        )
        g_repo = g_org.get_repo(repo.name)
...
@click.option(
    "--destroy-data",
    help="Confirm that you want to use this command that can destroy data.",
    is_flag=True,
)
def delete_repos(ctx, students=None, groups=None, destroy_data=False):
    ...
    if not destroy_data:
        click.secho(
            "Add the command line option --destroy-data to enable the delete-repos command."
        )
        exit(1)

    if not ghtt.config.get("enable-repo-delete", False):
        click.secho(
            'Add the line "enable-repo-delete: True" to your ghtt config to enable the delete-repos command.'
        )
        exit(1)
    ...
    click.secho(
        "# WARNING! Deleting student repositories. This will permanently remove all code and history of these repositories! For a less destructive way to remove a repository, see 'ghtt assignment rename-repo'.",
        fg="red",
    )
    ...
    for repo in repos.values():
        try:
            ...
            click.secho(
                "Warning: repository {}/{} does not exist; skipping..".format(
                    g_org.html_url, repo.name
                ),
                fg="yellow",
            )
...
def create_issues(ctx, path, yes, students=None, groups=None):
    ...
    for repo in repos.values():
        ...
        click.secho(
            "Generating issues in repo {}/{}".format(g_org.html_url, repo.name),
            fg="green",
        )

        issue_dicts: list[dict] | None = yaml.safe_load(
            render_template(issue_template_content, g_repo.ssh_url, repo)
        )
        ...
        for issue_dict in issue_dicts:
            ...
            if issue_type == "milestone":
                ...
                if due_on.tzinfo is None or due_on.tzinfo.utcoffset(due_on) is None:
                    ...
                    due_on = due_on.replace(tzinfo=tz.tzlocal()).astimezone(
                        tz.tzlocal()
                    )
                    assert (
                        due_on.tzinfo is not None
                        and due_on.tzinfo.utcoffset(due_on) is not None
                    )

                # find existing milestone with same title
                matching_milestone = [
                    existing_milestone
                    for existing_milestone in g_repo.get_milestones()
                    if existing_milestone.title == issue_dict.get("title")
                ]
                if len(matching_milestone) == 1:
                    if (
                        issue_dict.get("title") == matching_milestone[0].title
                        and issue_dict.get("description")
                        == matching_milestone[0].description
                        and due_on
                        == matching_milestone[0].due_on.replace(tzinfo=timezone.utc)
                    ):
                        click.secho(
                            "Skipping up to date milestone '{}'".format(
                        ...
                        click.secho(
                            "Updating milestone '{}'".format(issue_dict.get("title")),
                            fg="green",
                        )
                    ...
                    click.secho(
                        "Adding milestone '{}'".format(issue_dict.get("title")),
                        fg="green",
                    )
                    try:
                        ...
                        if (
                            len(e.data["errors"]) != 1
                            or e.data["errors"][0]["code"] != "already_exists"
                        ):
                            raise
                else:
                    # this is normally impossible
                    click.secho(
                        f"Skipping: There already exist {len(matching_milestone)} milestones "
                        f"with title '{issue_dict.get('title')}'",
                        fg="red",
                    )
                ...
                if milestone is not github.GithubObject.NotSet:
                    milestone = [
                        ms for ms in g_repo.get_milestones() if ms.title == milestone
                    ][0]

                # find existing issue with same title
                matching_issue = [
                    existing_issue
                    for existing_issue in g_repo.get_issues()
                    if existing_issue.title == issue_dict.get("title")
                ]

                if len(matching_issue) == 1:
                    same_labels = (
                        issue_dict.get("labels", []).sort()
                        == [l.name for l in matching_issue[0].labels].sort()
                    )
                    same_assignees = set(issue_dict.get("assignees")) == set(
                        [l.login for l in matching_issue[0].assignees]
                    )
                    same_milestone = (
                        issue_dict.get("milestone") == matching_issue[0].milestone
                        or matching_issue[0].milestone is not None
                        and issue_dict.get("milestone")
                        == matching_issue[0].milestone.title
                    )
                    if (
                        issue_dict.get("title") == matching_issue[0].title
                        and issue_dict.get("body") == matching_issue[0].body
                        and same_labels
                        and same_assignees
                        and same_milestone
                    ):
                        click.secho(
                            "Skipping up to date issue '{}'".format(
                                issue_dict.get("title")
                            ),
                            fg="green",
                        )
                        ...
                        click.secho(
                            "Updating issue with title '{}'".format(
                                issue_dict.get("title")
                            ),
                            fg="green",
                        )
                    ...
                    click.secho(
                        "Adding issue with title '{}'".format(issue_dict.get("title")),
                        fg="green",
                    )
                    try:
                        g_repo.create_issue(
                        ...
                        click.secho(
                            "Warning: could not create issue. Do the assignees have access to the repo? Skipping\n{}".format(
                                e
                            ),
                            fg="yellow",
                        )
                    ...
                    click.secho(
                        f"Skipping: There already exist {len(matching_issue)} issues "
                        f"with title '{issue_dict.get('title')}'",
                        fg="red",
                    )
    ...
    try:
        for repo in repos.values():
            try:
                ...
                summary.append(
                    (
                        ...
                        datetime.now(),
                        None,
                        "pull failed: repository not found",
                    )
                )
                continue
            ...
            try:
                ...
                subprocess.check_call(
                    ["git", "fetch", g_repo.ssh_url, "HEAD:{}".format(g_repo.name)],
                    cwd=source,
                )

                # subprocess.check_call(["git", "checkout", g_repo.name], cwd=source)
                timestamp = subprocess.check_output(
                    ["git", "log", g_repo.name, "-1", "--pretty=format:%ct"],
                    cwd=source,
                    universal_newlines=True,
                ).rstrip()
                commit_summary = subprocess.check_output(
                    ["git", "log", g_repo.name, "-1", "--pretty=format:%s"],
                    cwd=source,
                    universal_newlines=True,
                )
                committer = subprocess.check_output(
                    ["git", "log", g_repo.name, "-1", "--pretty=format:%an <%ae>"],
                    cwd=source,
                    universal_newlines=True,
                )
...
@assignment.command()
@click.pass_context
@click.option("--match", help='Regex to match repos. Example: "studnt-(.*)"')
@click.option(
    "--replace",
    help='New name for repos. This can contain \\1, \\2, ... to match the groups in the regex. Example: "student-\1"',
)
@click.option(
    "--yes", help="Process all repositories, without confirmation.", is_flag=True
)
def rename_repo(ctx, yes, match, replace):
    ...
    for g_repo in g_repos:
        ...
        if not asker.should_proceed(
            f"{g_repo.url} with name {g_repo.name} to {replacement}"
        ):
            continue
...
@assignment.command()
@click.pass_context
@click.option("--students", help="list of students")
@click.option(
    "--groups", help="Comma-separated list of group names. Defaults to all groups."
)
@click.option(
    "--read-only",
    help="Instead of granting push access, give only pull repository access (= read only). Issues can still be created and replied to.",
    is_flag=True,
)
@click.option(
    "--yes", help="Process all students/groups, without confirmation.", is_flag=True
...
def grant(ctx, yes, read_only, students=None, groups=None):
    ...
    click.secho(
        "# Granting students write permission to their repository..", fg="green"
    )
    click.secho("# Students: '{}'".format(students), fg="green")
    ...
    for repo in repos.values():
        ...
        if not asker.should_proceed(
            '{}" {} access to "{}'.format(
                '", "'.join([s.username for s in repo.students]), permission, repo.url
            )
        ):
            continue

        click.secho(
            "Granting students {} {} access to {}".format(
                [s.username for s in repo.students], permission, repo.url
            ),
            fg="green",
        )
        for student in repo.students:
            try:
                ...
                click.secho(
                    "Warning: {} ({}) does not have a GitHub account, skipping\n{}".format(
                        student.username, student.comment, e
                    ),
                    fg="yellow",
                )
            except github.GithubException as e:
                click.secho(
                    "Warning: could not grant {} ({}), skipping\n{}".format(
                        student.username, student.comment, e
                    ),
                    fg="yellow",
                )
...
def remove_grant(ctx, yes, students=None, groups=None):
    ...
    click.secho(
        "# Removing students write permission to their repository..", fg="green"
    )
    click.secho("# Students: '{}'".format(students), fg="green")
    ...
    for repo in repos.values():
        ...
        for invitation in g_repo.get_pending_invitations():
            if str(invitation.invitee) in [s.username for s in repo.students]:
                click.secho(
                    "Removing invitation for student '{}' for repo '{}'".format(
                        invitation.invitee, repo.name
                    ),
                    fg="green",
                )
                invitation.delete()
        ...
        for username in [s.username for s in repo.students]:
            click.secho(
                "Removing '{}' as collaborators from '{}'".format(username, repo.name),
                fg="green",
            )
